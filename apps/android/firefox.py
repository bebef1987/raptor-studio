...
class AbstractAndroidFirefox(object):
    def __init__(self, proxy, certutil, app_path):
        self.proxy = proxy
    ...
    def setup_device(self):
        ...
        if self.app_path:
            if self.device.is_app_installed(self.APP_NAME):
                print("killing app %s" % self.APP_NAME)
                self.device.uninstall_app(self.APP_NAME)
            self.device.install_app(apk_path=self.app_path)
    ...
    def app_information(self):
        version = None
        if self.app_path:
            version = mozversion.get_version(binary=self.app_path)
        return version
